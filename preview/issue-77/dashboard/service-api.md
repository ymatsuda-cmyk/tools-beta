...
コントローラーへの問い合わせは往復が重く、カードを更新するたびに
叩くと表示が待たされます。そこでダッシュボード用GASが定期的に
取り直した結果をスクリプトプロパティ`KAGGLE_SNAPSHOT`に置き、
カードはそれを読むだけにしています。

セットアップは、ダッシュボード用GASのエディタで
`setupKaggleSnapshotTrigger()`を**一度だけ**手で実行します。
以後5分おきに`refreshKaggleSnapshots()`が走ります。

取りに行く宛先は、カードから一度でも問い合わせがあった時点で
`KAGGLE_TARGETS`に覚えます。手で登録する必要はありません。

ためた値を使わず、その場で取り直すのは次の場合です。

- 画面右上の「今すぐ更新」を押したとき
- カード右上の更新ボタンを押したとき
- まだ一度もためていないとき
- トグルで起動/停止したとき（操作直後にためた値も入れ替える）
