/**
 * タグ語彙の集計と統合候補の抽出。
 * 動画ナレッジ(clipstock)の src/lib/vocab.js と同じ考え方。
 *
 * 統合候補にAIは使わない。「いつも一緒に付いている」は数えれば出るし、
 * 数えて出る答えを毎回LLMに聞くと、実行ごとに提案が変わって信用できなくなる。
 * 判断(まとめる / 別物として残す)は人が押す。
 */
...
export function mergeCandidates(items, dismissed = new Set()) {
  ...
  // 1回しか使われていないタグは、同じ文書に付いた全タグと共起100%になる。
...
const DISMISS_KEY = 'contents:vocabDismissed'
